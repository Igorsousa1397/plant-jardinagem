#!/usr/bin/env bash
# Plant Jardinagem — corrige layout dos campos Data/Duração no mobile
set -e
        ...
        <div className="flex gap-3">
          <div className="min-w-0 flex-1">
            <Field label="Data do serviço">
              <input type="date" value={data} onChange={(e) => setData(e.target.value)} className={`${inputClass} min-w-0`} />
            </Field>
          </div>
          <div className="min-w-0 flex-1">
            <Field label="Duração">
